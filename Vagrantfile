...
Vagrant.configure("2") do |config|
	...
	#Porta necessaria per logstash
	config.vm.network :forwarded_port, guest: 5000, host: 5000

	if is_windows?
		config.vm.network :forwarded_port, guest: 100, host: 100
	else
		config.vm.network :forwarded_port, guest: 100, host: 10080
	end
	...
	config.vm.provision :puppet, run: "always" do |puppet|
 	...
 	if !is_windows?
		config.trigger.after [:provision, :up, :reload] do
			system('echo "
				rdr pass on lo0 inet proto tcp from any to 127.0.0.1 port 100 -> 127.0.0.1 port 10080
				" | sudo pfctl -f - > /dev/null 2>&1; sudo pfctl -e; echo "==> Fowarding Ports: 100 -> 10080"')  
		end
